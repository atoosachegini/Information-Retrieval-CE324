...
parser.add_argument("crawl_count", help="number of articles to crawl", type=int, default=10, nargs='?')
...
class Article:
    def __init__(self, pfid, title, abstract, date, authors, rel_sub, citations, number_of_refs, references):
        self.id = pfid
        self.title = title
        self.abstract = abstract
        self.date = date
        self.authors = authors
        self.related_topics = rel_sub
        self.citation_count = citations
        self.reference_count = number_of_refs
...
def consume_json(j):
    ...
    citations = j['eccnt']
    number_of_refs = len(j['r'])
    references = j['r'][:10]
    rel_sub = []
    for sub in j['fos']:
        rel_sub.append(sub['dn'])

    # queue references for future crawl
    queue.extend(references)
    # save this article
    article = Article(id, title, abstract, pub_year, authors, rel_sub, citations, number_of_refs, references)
    data.append(article)
    article.save()
    # saw this article
    seen.add(id)
...
with open('CrawledPapertest.json', 'w') as f:
